...
namespace Solvers
{
	...
	template<int dimension, typename CostFunction = std::function<double (const boost::numeric::ublas::c_vector<double, dimension> &)> >
	...
	public:
		...
		struct SimplexPoint
		{
            ...
            friend std::ostream &operator<< (std::ostream &output, const SimplexPoint &p)
            {
		...
		NmSimplexSolver( CostFunction f, double step, double epsilon, bool doReport = false)
			:f( f),step( step),  epsilon( epsilon), lastIterationCount( 0), lastCostValue( 0.0), doReport(doReport)
		{
		}
		...
		Point FindMinimun( Point startingPoint,  unsigned int maxIterations = 1000)
		{
			...
			const unsigned int best = 0, secondWorst = dimension - 1, worst = dimension;
			...
			do
			{
				...
				SimplexPoint reflected = PointAndValue( centroid + alpha * (centroid - simplex[worst].position));
				...
				{
					SimplexPoint expanded = PointAndValue( centroid + gamma * (centroid - simplex[worst].position));
				...
				else // reflected >= simplex[secondWorst]
				{
					...
					{
						SimplexPoint contracted = PointAndValue( centroid + beta * (reflected.position - centroid));
					...
					{
						SimplexPoint contracted = PointAndValue( centroid + beta * (simplex[worst].position - centroid));
				...
				if (doReplace)
				{
                    ...
                    std::inplace_merge( simplex.begin(), simplex.end() -1, simplex.end());
				}
				else
				{
			...
			}  while ( ++iterationCount <= maxIterations &&  (simplex[worst].value - simplex[best].value > epsilon));
	...
	private:
		...
		void Grow( Simplex &simplex, double factor) const
		{
			...
			{
				*i = PointAndValue( firstPoint + factor * (i->position - firstPoint));
			}
		}
		...
		Simplex StartingSimplex( const Point &startingPoint) const
		{
			...
			{
				simplex[i] = PointAndValue( startingPoint + step * unit_vector<double>( dimension, i -1));
			}
